...
public final class VexTeleportTransferHandler implements MessageHandler<TeleportTransferRequest> {
    ...
    public void handle(final TeleportTransferRequest request, final MessageContext context) {
        ...
        RegisteredServer destination = proxy.getServer(request.destination().server().value()).orElse(null);
        ...
        if (player.getCurrentServer().map(connection -> connection.getServer().equals(destination)).orElse(false)) {
    ...
    private void forwardArrival(final TeleportTransferRequest request, final String sourceServer) {
        DeliveryResult delivery = messages.send(
            MessageTarget.server(request.destination().server().value()),
            TeleportMessages.ARRIVAL,
            new TeleportArrival(request.requestId(), request.playerId(), request.destination(), sourceServer)
        );
