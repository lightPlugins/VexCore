...
public final class VexPlayerDirectoryRequestHandler implements MessageHandler<PlayerDirectoryRequest> {
    ...
    public void handle(final PlayerDirectoryRequest request, final MessageContext context) {
        ...
        String serverId = player.flatMap(connected -> connected.getCurrentServer())
        ...
        messages.send(
            ...
            new PlayerDirectoryResponse(request.requestId(), request.playerId(), playerName, serverId)
        );
    }
}
