...
public final class VexInteractionTrackerService implements InteractionTrackerService {
    ...
    public void track(final TrackedInteraction interaction) {
        interactions.put(
            key(interaction.getHandle().getViewerId(), interaction.getHandle().getEntityId()),
            interaction
        );
    }
    ...
    @Override
    public boolean setInputBlocked(final ServiceOwner owner, final UUID viewerId, final boolean blocked) {
        if (blocked) {
            inputBlocks.computeIfAbsent(viewerId, ignored -> ConcurrentHashMap.newKeySet()).add(owner);
        } else {
            inputBlocks.computeIfPresent(
                viewerId,
                (ignored, owners) -> {
                    owners.remove(owner);
    ...
    public Optional<TrackedInteraction> remove(final FakeInteractionHandle handle) {
        return Optional.ofNullable(interactions.remove(key(handle.getViewerId(), handle.getEntityId())));
    }

    @Override
    public Collection<TrackedInteraction> findOwned(final ServiceOwner owner, final UUID viewerId) {
    ...
    public Collection<TrackedInteraction> removeOwned(final ServiceOwner owner) {
        Collection<TrackedInteraction> owned = interactions.values()
            .stream()
            .filter(interaction -> interaction.getHandle().getOwner().equals(owner))
            .toList();
    ...
    @Override
    public Collection<TrackedInteraction> removeViewer(final UUID viewerId, final DisplayLifecycle lifecycle) {
        return removeMatching(viewerId, lifecycle);
    }

    private Collection<TrackedInteraction> removeMatching(final UUID viewerId, final DisplayLifecycle lifecycle) {
        Collection<TrackedInteraction> removed = interactions.values()
            .stream()
            .filter(interaction -> interaction.getHandle().getViewerId().equals(viewerId))
