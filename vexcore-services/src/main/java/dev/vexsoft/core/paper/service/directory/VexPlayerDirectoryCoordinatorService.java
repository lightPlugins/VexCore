...
public final class VexPlayerDirectoryCoordinatorService implements PlayerDirectoryCoordinatorService, AutoCloseable {
    ...
    public VexPlayerDirectoryCoordinatorService(final VexServiceRegistry services) {
        ...
        onlinePlayerSnapshots = checked.require(CacheService.class).createAsync(
            ...
            VexCacheOptions.builder().maximumSize(1).expireAfterWrite(Duration.ofSeconds(2)).build(),
    ...
    public void complete(final PlayerDirectoryResponse response) {
        ...
        CompletableFuture<Optional<NetworkPlayer>> result = pending.remove(checkedResponse.requestId());
        ...
        Optional<NetworkPlayer> player =
            checkedResponse.serverId().isBlank() || checkedResponse.playerName().isBlank() ? Optional.empty()
    ...
    public void complete(final PlayerDirectoryListResponse response) {
        ...
        CompletableFuture<List<NetworkPlayer>> result = pendingLists.remove(checkedResponse.requestId());
    ...
    private CompletableFuture<List<NetworkPlayer>> requestOnlinePlayers() {
        ...
        return result.completeOnTimeout(lastOnlinePlayers, LOOKUP_TIMEOUT_SECONDS, TimeUnit.SECONDS)
            .thenApply(players -> {
