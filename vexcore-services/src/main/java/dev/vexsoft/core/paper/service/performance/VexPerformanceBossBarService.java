...
/** Displays localized server-performance boss bars for subscribed players. */
@Dependencies({ServerPerformanceService.class, ScheduleService.class, LocalizationService.class, PlayerService.class})
public final class VexPerformanceBossBarService implements PerformanceBossBarService, AutoCloseable {
    ...
    public void show(final Player player) {
        ...
        BossBar bossBar = BossBar.bossBar(Component.empty(), 0.0F, BossBar.Color.GREEN, BossBar.Overlay.PROGRESS);
    ...
    private void updateAll() {
        ...
        updatesUntilPingRefresh = refreshPing ? PING_UPDATE_INTERVALS - 1 : updatesUntilPingRefresh - 1;
    ...
    private void update(final Player player, final BossBar bossBar, final boolean refreshPing) {
    ...
    private Map<String, String> replacements(
        ...
        if (!snapshot.isAvailable()) {
            return Map.of("tps", "N/A", "mspt", "N/A", "ping", ping);
        }

        return Map.of("tps", format(snapshot.getCurrentTps()), "mspt", format(snapshot.getAverageMspt()), "ping", ping);
    }
