...
/** Samples server tick performance and exposes the latest calculated snapshot. */
@Dependencies({PlatformService.class, ScheduleService.class})
public final class VexServerPerformanceService implements ServerPerformanceService, AutoCloseable {
    ...
    private void sample() {
        snapshot = PerformanceSnapshotCalculator.calculate(server.getTickTimes(), System.currentTimeMillis());
    }
}
