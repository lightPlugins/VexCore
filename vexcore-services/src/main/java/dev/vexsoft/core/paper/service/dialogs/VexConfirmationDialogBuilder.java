...
public final class VexConfirmationDialogBuilder extends
    AbstractDialogBuilder<Boolean, ConfirmationDialogBuilder> implements ConfirmationDialogBuilder {
    ...
    protected Dialog buildDialog(final DialogSession<Boolean> session) {
        return Dialog.create(factory -> factory.empty().base(base(List.of())).type(DialogType.confirmation(
    ...
    private void complete(final DialogSession<Boolean> session, final boolean value, final DialogResultType type) {
        scheduler.runFor(
            player,
