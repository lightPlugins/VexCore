...
class PerformanceSnapshotCalculator {
    ...
    static ServerPerformanceSnapshot calculate(final long[] tickTimes, final long sampledAt) {
        ...
        double tps = average <= TICK_BUDGET_MILLIS ? TARGET_TPS : Math.min(TARGET_TPS, 1_000.0D / average);

        return new ServerPerformanceSnapshot(true, average, minimum, maximum, tps, sampledAt, state(average));
    }

    static ServerPerformanceSnapshot unavailable(final long sampledAt) {
        return new ServerPerformanceSnapshot(false, 0.0D, 0.0D, 0.0D, 0.0D, sampledAt, PerformanceState.UNAVAILABLE);
    }
