...
/** Tracks active player dialogs across owners and completes or retires their sessions. */
@Dependencies
public final class VexDialogCoordinatorService implements DialogCoordinatorService, AutoCloseable {
    ...
    public <T> DialogSession<T> begin(final ServiceOwner owner, final UUID playerId) {
        DialogSession<T> session =
            new DialogSession<>(Objects.requireNonNull(owner, "owner"), Objects.requireNonNull(playerId, "playerId"));
    ...
    @Override
    public <T> boolean complete(final DialogSession<T> session, final DialogResult<T> result) {
    ...
    @Override
    public boolean close(final ServiceOwner owner, final UUID playerId, final DialogResultType type) {
    ...
    @Override
    public void close() {
        sessions.values().stream().toList().forEach(session -> complete(session, DialogResultType.PLUGIN_DISABLED));
    }
}
