...
/** Tracks pending proxy pings and completes them when responses arrive or requests expire. */
...
public final class VexProxyPingService implements ProxyPingService, AutoCloseable {
    ...
    public UUID begin(final Player player) {
        ...
        schedules.runForLater(checkedPlayer, TIMEOUT_TICKS, () -> timeout(requestId), () -> pending.remove(requestId))
            .ifPresentOrElse(
                ping::setTimeoutTask,
