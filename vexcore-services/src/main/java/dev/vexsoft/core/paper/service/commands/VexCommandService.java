...
/** Builds owner-specific command trees from annotated classes and resolves command arguments. */
@Dependencies
public final class VexCommandService implements CommandService {
    ...
    public VexCommandService(final VexServiceRegistry services) {
        ...
        plugin.getLifecycleManager().registerEventHandler(LifecycleEvents.COMMANDS, event -> bind(event.registrar()));
    }
    ...
    private synchronized void bind(final Commands registrar) {
        ...
        for (CommandTree tree : commands.values()) {
            ...
            build(tree.getRoot(), rootBuilder, tree.getAnnotation().permission(), tree.getAnnotation().playerOnly());
            registrar.register(
                rootBuilder.build(),
    ...
    private void addPath(final CommandNode root, final Object handler, final Method method, final Command command) {
    ...
    private void setExecution(final CommandNode node, final CommandExecution execution, final String path) {
    ...
    private void build(
        ...
        if (execution != null) {
            builder.executes(context -> invoke(execution, context, rootPermission, rootPlayerOnly));
        }

        for (CommandNode child : node.getChildren().values()) {
            ArgumentBuilder<CommandSourceStack, ?> childBuilder =
                child.getSpec() == null ? literal(child.getName()) : argumentBuilder(child.getName(), child.getSpec());
    ...
    private ArgumentBuilder<CommandSourceStack, ?> argumentBuilder(final String name, final ArgumentSpec spec) {
        ...
        if (customArgument != null) {
            ...
            builder = argument(name, spec.isGreedy() ? StringArgumentType.greedyString() : StringArgumentType.word());
        } else {
    ...
    private void applySuggestions(
        ...
        if (spec.getSuggestionType() != null) {
            ...
            builder.suggests((context, suggestions) -> provider.suggest(
    ...
    private int invoke(
        ...
        if (!canExecute(context.getSource(), execution, rootPermission, rootPlayerOnly)) {
            throw CommandSyntaxException.BUILT_IN_EXCEPTIONS.dispatcherUnknownCommand().create();
        }

        try {
            Object result = execution.getMethod()
                .invoke(
    ...
    private Object[] invocationArguments(final Method method, final CommandContext<CommandSourceStack> context) {
    ...
    private static Object enumValue(final Class<?> type, final String value) {
        @SuppressWarnings({"rawtypes", "unchecked"}) Object result =
            Enum.valueOf((Class<? extends Enum>) type.asSubclass(Enum.class), value.toUpperCase(Locale.ROOT));
    ...
    private interface ArgumentReader {
        ...
        /** Reads one named argument from the Brigadier command context. */
    ...
    private boolean canAccessNode(
        ...
        if (node.getExecution() != null && canExecute(source, node.getExecution(), rootPermission, rootPlayerOnly)) {
            return true;
        }
    ...
    private boolean canExecute(
        ...
        return command.permission().isBlank() || source.getSender().hasPermission(command.permission());
    }
    ...
    private boolean canUseRoot(
        ...
        return rootPermission.isBlank() || source.getSender().hasPermission(rootPermission);
    }
    ...
    private boolean supportsStringArgument(final Class<?> type) {
        return type == String.class || type == Player.class || type == UUID.class || type == Duration.class
            || type.isEnum() || argumentTypes.containsKey(type);
    }
    ...
    private void registerArgumentInstance(final CommandArgumentType<?> argumentType) {
        ...
        Class<?> valueType = Objects.requireNonNull(checkedArgument.getValueType(), "Command argument value type");
        ...
        if (existing != null && existing.getClass() != checkedArgument.getClass()) {
            throw new IllegalStateException("Command argument type is already registered: " + valueType.getName());
        }
    }
    ...
    private static class CommandTree {
        ...
        private void requireCompatible(final CommandRoot other) {
            if (!annotation.description().equals(other.description()) || !annotation.permission()
                .equals(other.permission()) || annotation.playerOnly() != other.playerOnly() || !Arrays.equals(
                annotation.aliases(),
                other.aliases()
            )) {
