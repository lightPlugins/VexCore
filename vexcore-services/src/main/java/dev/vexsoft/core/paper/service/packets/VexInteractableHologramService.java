...
public final class VexInteractableHologramService implements InteractableHologramService, AutoCloseable {

    private final ServiceOwner owner;
    private final TextDisplayPacketService textDisplays;
    private final InteractionPacketService interactions;
    private final Map<InteractableHologramHandle, HologramState> holograms = new ConcurrentHashMap<>();
    ...
    @Override
    public InteractableHologramHandle spawn(final Player viewer, final InteractableHologramRequest request) {
        ...
        FakeInteractionHandle interactionHandle = interactions.spawn(
            viewer,
            FakeInteractionRequest.builder(interactionLocation(text.getLocation(), request.getHitboxOffset()))
                .width(request.getHitboxWidth())
                .height(request.getHitboxHeight())
                .lifecycle(text.getLifecycle())
                .interactHandler(interaction -> request.getInteractHandler()
                    .handle(toHologramInteraction(textHandle, interaction)))
                .build()
        );
        ...
        holograms.put(
            handle,
            new HologramState(textHandle, interactionHandle, text.getLocation(), request.getHitboxOffset())
        );
    ...
    @Override
    public void update(final InteractableHologramHandle handle, final FakeTextDisplayUpdate update) {
        state(handle).ifPresent(state -> textDisplays.update(state.textHandle(), update));
    }

    @Override
    public void updateHitbox(final InteractableHologramHandle handle, final float width, final float height) {
        state(handle).ifPresent(state -> interactions.updateHitbox(state.interactionHandle(), width, height));
    }

    @Override
    public void updateHitboxOffset(final InteractableHologramHandle handle, final Vector offset) {
        state(handle).ifPresent(state -> {
            state.hitboxOffset(offset);
            interactions.teleport(state.interactionHandle(), interactionLocation(state.location(), offset));
        });
    }
    ...
    public void teleport(final InteractableHologramHandle handle, final Location location) {
        state(handle).ifPresent(state -> {
            ...
            interactions.teleport(state.interactionHandle(), interactionLocation(location, state.hitboxOffset()));
        });
    }

    @Override
    public void teleportDisplay(final InteractableHologramHandle handle, final Location location) {
    ...
    private static HologramInteraction toHologramInteraction(
        ...
        return new HologramInteraction(
            ...
            interaction.getInteractionType() == FakeInteractionType.LEFT_CLICK ? HologramInteractionType.LEFT_CLICK
