...
/** Publishes Bukkit language-change events using the caller's current thread context. */
@Dependencies
public final class VexLanguageChangeDispatcherService implements LanguageChangeDispatcherService {
    ...
    @Override
    public void dispatch(final VexPlayer player, final Language previousLanguage, final Language newLanguage) {
        Bukkit.getPluginManager()
            .callEvent(new VexPlayerLanguageChangeEvent(
                player,
                previousLanguage,
                newLanguage,
