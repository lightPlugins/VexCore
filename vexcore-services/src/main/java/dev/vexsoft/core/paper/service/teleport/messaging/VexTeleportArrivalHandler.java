...
public final class VexTeleportArrivalHandler implements MessageHandler<TeleportArrival> {
    ...
    private void sendCompletion(final TeleportArrival arrival, final TeleportResult result, final Throwable throwable) {
        TeleportCompletion completion =
            throwable == null ? new TeleportCompletion(arrival.requestId(), result.status(), result.message())
        ...
        messages.send(MessageTarget.server(arrival.sourceServer()), TeleportMessages.COMPLETION, completion);
    }
}
