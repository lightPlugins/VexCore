...
public final class VexActionBarCoordinatorService implements ActionBarCoordinatorService, AutoCloseable {
    ...
    public void showTemporary(
        ...
        state.channels().showTemporary(owner, channel, component, priority, sequence.incrementAndGet(), expiresAt);
        refreshSoon(player, state);
    }
    ...
    private ManagedState state(final Player player) {
        ...
        ManagedState state =
            states.computeIfAbsent(checkedPlayer.getUniqueId(), ignored -> new ManagedState(checkedPlayer));
    ...
    private static final class ManagedState {
        ...
        synchronized void start(final ScheduleService schedules, final Runnable update, final Runnable retired) {
            if (task != null || starting) {
                return;
            }

            starting = true;
            task = schedules.runForTimer(player, 1L, UPDATE_INTERVAL_TICKS, update, retired).orElse(null);
            starting = false;
        }
