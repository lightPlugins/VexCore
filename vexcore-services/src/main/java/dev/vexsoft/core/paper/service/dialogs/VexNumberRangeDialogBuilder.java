...
public final class VexNumberRangeDialogBuilder extends AbstractDialogBuilder<Float, NumberRangeDialogBuilder> implements
    NumberRangeDialogBuilder {
    ...
    protected Dialog buildDialog(final DialogSession<Float> session) {
        ...
        return Dialog.create(factory -> factory.empty().base(base(List.of(input))).type(DialogType.confirmation(
    ...
    private void submit(final DialogSession<Float> session, final Float value) {
        scheduler.runFor(
            ...
            () -> {
                ...
                coordinator.complete(session, DialogResult.value(DialogResultType.CONFIRMED, value));
            },
