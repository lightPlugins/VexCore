...
/** Schedules and tracks owner-bound global, region, entity, and asynchronous tasks. */
...
public final class VexScheduleService implements ScheduleService, AutoCloseable {
    ...
    @Override
    public VexTask runGlobalTimer(final long initialDelayTicks, final long periodTicks, final Runnable task) {
        ensureOpen();
        requirePositive(initialDelayTicks, "initialDelayTicks");
        requirePositive(periodTicks, "periodTicks");

        return track(server.getGlobalRegionScheduler()
            .runAtFixedRate(
                owner,
                callback(task),
                initialDelayTicks,
    ...
    public VexTask runAtLater(final Location location, final long delayTicks, final Runnable task) {
        ...
        return track(server.getRegionScheduler()
            .runDelayed(
                owner,
    ...
    @Override
    public Optional<VexTask> runFor(final Entity entity, final Runnable task, final Runnable retired) {
        ensureOpen();
        AtomicReference<ScheduledTask> reference = new AtomicReference<>();
        ScheduledTask scheduled = Objects.requireNonNull(entity, "entity")
            .getScheduler()
            .run(
                owner,
                callback(task),
    ...
    public VexTask runAsyncLater(final Duration delay, final Runnable task) {
        ...
        return track(server.getAsyncScheduler().runDelayed(owner, callback(task), delayNanos, TimeUnit.NANOSECONDS));
    }

    @Override
    public VexTask runAsyncTimer(final Duration initialDelay, final Duration interval, final Runnable task) {
        ensureOpen();
        ...
        return track(server.getAsyncScheduler()
            .runAtFixedRate(
                owner,
                callback(task),
                initialDelayNanos,
    ...
    private Runnable retiredCallback(final Runnable retired, final AtomicReference<ScheduledTask> reference) {
        return () -> {
            try {
                if (retired != null) {
                    retired.run();
