...
public final class VexPlaceholderApiBridgeService implements PlaceholderApiBridgeService, Listener, AutoCloseable {
    ...
    public synchronized void enable() {
        ...
        if (created.register()) {
            expansion = created;
            plugin.getLogger().info("PlaceholderAPI support enabled with expansion '" + created.getIdentifier() + "'");
        } else {
            plugin.getLogger()
                .warning("Could not register the " + created.getIdentifier() + " PlaceholderAPI expansion");
        }
    }
