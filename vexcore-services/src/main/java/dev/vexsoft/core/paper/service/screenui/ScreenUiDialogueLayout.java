...
public final class ScreenUiDialogueLayout {
    ...
    public static void validate(DialoguePanelLayout layout, ScreenUiVersionDefinition version) {
        ScreenUiRenderer.texture(
            UiTexture.PANEL,
            TextureLayout.builder().anchor(layout.anchor()).x(layout.panelX()).y(layout.panelY()).build(),
            version
        );
        version.textFont(layout.anchor(), layout.nameY());
        version.textFont(layout.anchor(), layout.hintY());

        for (int lineIndex = 0; lineIndex < layout.linesPerPage(); lineIndex++) {
            version.textFont(layout.anchor(), layout.bodyY() + lineIndex * (12 + layout.lineSpacing()));
        }
    }

    public static PreparedDialogue prepare(
        Component speaker,
        List<Component> paragraphs,
        DialoguePanelLayout layout,
        ...
        var nameLines = ScreenUiRenderer.layout(
            List.of(normalizedName),
            TextBlockLayout.builder().maxWidth(layout.textWidth()).overflow(TextOverflow.ELLIPSIS).build()
        );
