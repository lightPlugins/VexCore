...
public final class VexSidebarCoordinatorService implements SidebarCoordinatorService, AutoCloseable {

    private static final long NANOS_PER_TICK = 50_000_000L;
    private static final String[] ENTRIES =
        {"vex_line_0", "vex_line_1", "vex_line_2", "vex_line_3", "vex_line_4", "vex_line_5", "vex_line_6", "vex_line_7", "vex_line_8", "vex_line_9", "vex_line_10", "vex_line_11", "vex_line_12", "vex_line_13", "vex_line_14"};
    ...
    public void showTemporary(
        ...
        state.channels.showTemporary(owner, channel, frame, priority, sequence.incrementAndGet(), expires);
        refreshSoon(state);
        schedules.runForLater(state.player, durationTicks, () -> refresh(state), () -> retire(state)).ifPresentOrElse(
            ignored -> {
            },
            () -> retire(state)
        );
    ...
    @Override
    public void clearPersistent(final ServiceOwner owner, final Player player, final String channel) {
    ...
    @Override
    public void clearTemporary(final ServiceOwner owner, final Player player, final String channel) {
        ManagedState state = states.get(player.getUniqueId());
    ...
    private ManagedState state(final Player player) {
        ...
        ManagedState state = states.computeIfAbsent(checked.getUniqueId(), ignored -> new ManagedState(checked));
    ...
    private void refreshSoon(final ManagedState state) {
        schedules.runFor(state.player, () -> refresh(state), () -> retire(state)).ifPresentOrElse(
            ignored -> {
            },
            () -> retire(state)
        );
    ...
    private static final class ManagedState {
        ...
        private void ensureScoreboard() {
            ...
            scoreboard = Objects.requireNonNull(Bukkit.getScoreboardManager(), "scoreboard manager").getNewScoreboard();
        ...
        private synchronized void close() {
            if (scoreboard != null && player.isOnline() && player.getScoreboard() == scoreboard) {
                Scoreboard fallback =
                    previous == null ? Objects.requireNonNull(Bukkit.getScoreboardManager()).getMainScoreboard()
                        : previous;
