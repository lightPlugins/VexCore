...
public final class ScreenUiRenderer {
  ...
  public static List<Line> layout(final List<Component> input, final TextBlockLayout layout) {
    ...
    int height = wrapped.isEmpty() ? 0 : wrapped.size() * (ScreenUiFont.HEIGHT + layout.lineSpacing()) - layout.lineSpacing();
  ...
  private static void flatten(Component component, Style inherited, List<List<Glyph>> lines,
  int[] budget, int depth) {
    ...
    if (!(component instanceof TextComponent text)) {
      throw new IllegalArgumentException("Resolve translations and non-text components before displaying UI");
    }
    ...
    if (style.font() != null && !style.font().equals(Key.key("minecraft:default")) && !icon) {
      throw new IllegalArgumentException("Prototype text blocks require the built-in UI font; use textureBlock for icons");
    }
    ...
    for (int point : text.content().codePoints().toArray()) {
      ...
      if (!(icon ? point == 0xE001 : point >= 32 && point <= 126 || point >= 160 && point <= 255
      ...
      if (icon) style = style.decoration(TextDecoration.BOLD, false);
  ...
  private static void wrap(List<Glyph> paragraph, TextBlockLayout layout, List<List<Glyph>> output) {
    ...
    while (!remaining.isEmpty()) {
      ...
      if (layout.overflow() == TextOverflow.ELLIPSIS) {
        Glyph dot = new Glyph(".", remaining.getFirst().style(),
        ScreenUiFont.advance('.') + (remaining.getFirst().style().decoration(TextDecoration.BOLD) == TextDecoration.State.TRUE ? 1 : 0));
  ...
  public static Component text(List<Component> input, TextBlockLayout layout, ScreenUiVersionDefinition version) {
    var output = Component.text();
    for (Line line : layout(input, layout)) {
      Key font = layout.anchor() == null ? Key.key("vexcore:ui/text/y_" + line.y())
      ...
      for (Glyph glyph : line.glyphs()) {
        if (layout.anchor() != null) {
          for (TextDecoration decoration : List.of(TextDecoration.ITALIC, TextDecoration.UNDERLINED, TextDecoration.STRIKETHROUGH)) {
            if (glyph.style().decoration(decoration) == TextDecoration.State.TRUE) {
              throw new IllegalArgumentException("Anchored prototype does not support " + decoration);
            }
          }
        }
        if (current != null && !current.equals(glyph.style())) {
          output.append(textRun(run, animatedStyle(current, layout.animation()), runFont(current, font, layout, line.y(), version), layout.anchor() != null));
          run.setLength(0);
        }
  ...
  private static Style animatedStyle(Style style, dev.vexsoft.core.paper.screenui.UiAnimation animation) {
    if (animation == null) return style;
    var color = style.color() == null ? NamedTextColor.WHITE : style.color();
    int rgb = ((color.red() * 7 + 127) / 255 << 6)
        | ((color.green() * 7 + 127) / 255 << 3) | (color.blue() * 7 + 127) / 255;
    return style.color(net.kyori.adventure.text.format.TextColor.color((rgb << 15) | animation.startTick()));
  }

  private static Key runFont(Style style, Key fallback, TextBlockLayout layout, int y, ScreenUiVersionDefinition version) {
    if (style.font() == null || !style.font().value().startsWith("icon/")) return fallback;
    if (layout.anchor() == null) throw new IllegalArgumentException("UI icons require a screen anchor");
    return version.iconFont(style.font(), layout.anchor(), y, layout.animation() != null, layout.scale());
  }

  public static Component texture(UiTexture texture, TextureLayout layout, ScreenUiVersionDefinition version) {
    ...
    Key font = layout.anchor() == null ? Key.key(texture.fontPrefix().namespace(), texture.fontPrefix().value() + "/y_" + y)
        : Objects.requireNonNull(version, "Anchored UI requires a version adapter")
            .textureFont(texture, layout.anchor(), y, layout.animation() != null, layout.scale());
    Component glyph = Component.text(Character.toString(texture.codePoint()))
    ...
    return Component.empty().append(space(x))
    .append(glyph)
  ...
  public static Component space(int advance) {
    ...
    return Component.text(text.toString()).font(Key.key("vexcore:ui/space"))
