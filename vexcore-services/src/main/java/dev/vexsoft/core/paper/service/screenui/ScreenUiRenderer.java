...
public final class ScreenUiRenderer {
    ...
    public static List<Line> layout(final List<Component> input, final TextBlockLayout layout) {
        ...
        for (int lineIndex = 0; lineIndex < wrapped.size(); lineIndex++) {
            List<Glyph> glyphs = List.copyOf(wrapped.get(lineIndex));
            int width = width(glyphs);
            int y = top + lineIndex * (ScreenUiFont.HEIGHT + layout.lineSpacing());
            ...
            if (layout.overflow() == TextOverflow.ELLIPSIS) {
                Glyph dot = new Glyph(
                    ...
                    ScreenUiFont.advance('.') + (
                        remaining.getFirst().style().decoration(TextDecoration.BOLD) == TextDecoration.State.TRUE ? 1
                            : 0)
                );
        ...
        for (Line line : layout(input, layout)) {
            ...
            for (Glyph glyph : line.glyphs()) {
                if (layout.anchor() != null) {
                    for (TextDecoration decoration : List.of(
    ...
    public static Component texture(UiTexture texture, TextureLayout layout, ScreenUiVersionDefinition version) {
        ...
        Key font = layout.anchor() == null ? Key.key(
            texture.fontPrefix().namespace(),
            texture.fontPrefix().value() + "/y_" + y
        ) : Objects.requireNonNull(version, "Anchored UI requires a version adapter")
            .textureFont(
                texture,
                layout.anchor(),
        ...
        return Component.empty().append(space(x)).append(glyph).append(space(-x - texture.glyphAdvance()));
    }
