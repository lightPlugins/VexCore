...
public final class VexInteractionPacketService implements InteractionPacketService, AutoCloseable {
    ...
    @Override
    public FakeInteractionHandle spawn(final Player viewer, final FakeInteractionRequest request) {
        FakeInteractionHandle handle =
            new FakeInteractionHandle(owner, viewer.getUniqueId(), adapter.allocateEntityId(), UUID.randomUUID());
        ...
        tracker.track(new TrackedInteraction(handle, request.getInteractHandler(), request.getLifecycle()));
    ...
    @Override
    public void updateHitbox(final FakeInteractionHandle handle, final float width, final float height) {
    ...
    public void removeAll(final Player viewer) {
        tracker.findOwned(owner, viewer.getUniqueId())
            .stream()
