...
public final class VexDialoguePanel implements DialoguePanel {
        ...
        for (int lineIndex = 0; lineIndex < layout.linesPerPage(); lineIndex++) {
            screen.textBlock(
                ...
                TextBlockLayout.builder()
                    .anchor(layout.anchor())
                    .x(layout.textX())
                    .y(layout.bodyY() + lineIndex * (12 + layout.lineSpacing()))
                    .maxWidth(layout.textWidth())
                    .overflow(TextOverflow.REJECT)
                    .layer(10)
                    .build()
    ...
    @Override
    public synchronized void show(int page, int visibleCodePoints) {
        if (screen.isClosed()) {
        ...
        for (int lineIndex = 0; lineIndex < dialogue.layout().linesPerPage(); lineIndex++) {
            Component text = lineIndex < content.lines().size() ? reveal(content.lines().get(lineIndex), remaining)
                : Component.empty();

            screen.setLines("line-" + lineIndex, List.of(text));
        }
