...
public final class VexSignalRegistryService implements SignalRegistryService {
    ...
    private final Map<Class<? extends VexSignal>, List<RegisteredListener>> typeListeners = new HashMap<>();
    ...
    public SignalDispatchResult publish(final VexSignal signal) {
        ...
        for (RegisteredListener listener : listeners) {
            ...
            try {
                ...
                logger.log(
                    Level.SEVERE,
                    "Signal listener owned by " + listener.owner().getServiceOwnerName() + " failed for "
                        + checkedSignal.getKey().asString(),
                    exception
                );
            }
        }
    ...
    private SignalSubscription register(
        ...
        RegisteredListener registration =
            new RegisteredListener(registrationIds.incrementAndGet(), owner, invoker, signalType, signalKey, this);
    ...
    private VexSignal validate(final VexSignal signal) {
        ...
        Objects.requireNonNull(checked.getAttributes(), "signal attributes");
    ...
    private <K> void removeOwned(final Map<K, List<RegisteredListener>> listeners, final ServiceOwner owner) {
    ...
    private interface SignalInvoker {
        ...
        /** Invokes one registered signal listener with the validated signal. */
