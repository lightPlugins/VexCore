...
public final class ScreenUiDialogueTest {
    ...
    public void sentencesStayOnSeparatePagesAndOversizedSentencesRemainReadable() {
        ...
        var longSentence = ScreenUiDialogueLayout.prepare(
            Component.text("NPC"),
    ...
    @Test
    public void wrapsBeforeRevealAndPreservesPageBoundariesAndStyles() {
        var prepared = ScreenUiDialogueLayout.prepare(
            Component.text("Researcher"),
            List.of(
                Component.text("First\nSecond\nThird\nFourth").decorate(TextDecoration.BOLD),
                Component.text("Last")
            ),
            DialoguePanelLayout.defaults(),
            new V26_2ScreenUiVersionDefinition()
        ...
        assertEquals(
            ...
            prepared.pages().getFirst().lines().stream().map(PLAIN::serialize).toList()
        );
        ...
        assertTrue(ScreenUiRenderer.layout(List.of(prepared.speaker()), glyphLayout)
            .getFirst()
            .glyphs()
            .stream()
            .allMatch(glyph -> glyph.style().decoration(TextDecoration.BOLD) == TextDecoration.State.TRUE));
        assertTrue(ScreenUiRenderer.layout(prepared.pages().getFirst().lines(), glyphLayout)
            .stream()
        ...
        ScreenUi screen = (ScreenUi) Proxy.newProxyInstance(
            ...
            new Class<?>[]{ScreenUi.class},
            (proxy, method, args) -> {
                switch (method.getName()) {
                    case "textBlock" -> positions.put((String) args[0], (TextBlockLayout) args[2]);
                    case "setLines" -> {
                        content.put((String) args[0], (List<Component>) args[1]);
                        updates[0]++;
                    }
                    case "isClosed" -> {
                        return closed[0];
    ...
    @Test
    public void validatesGeometryAndSupportsArbitraryOriginsBeforeOpening() {
