...
final class ScreenUiShaderContractTest {
  ...
  @Test void everyDebugAnchorResolvesToAnExistingFontAndDecodesAfterFloatConversion() throws Exception {
    Properties protocol = new Properties();
    try (var input = Files.newInputStream(pack.resolve("vexcore-screen-ui.properties"))) { protocol.load(input); }
    ...
    for (ScreenAnchor anchor : ScreenAnchor.values()) {
      ...
      var layout = TextBlockLayout.builder().anchor(anchor).y(row == 0 ? 8 : row == 1 ? 0 : -8)
          .verticalAlignment(VerticalAlignment.values()[row]).build();
      ...
      assertTrue(Files.exists(pack.resolve("assets/vexcore/font/" + version.textureFont(UiTexture.PANEL, anchor, panelY).value() + ".json")));
    }
  ...
  @Test void markedAtlasPreservesInkAndMetricsAndContainsBothSignaturePixels() throws Exception {
    ...
    for (int point = 33; point <= 255; point++) {
      if (point > 126 && point < 161) continue;
      int col = (point - 32) % 32;
      int row = (point - 32) / 32;
      ...
      for (int x = 0; x < 16; x++) {
        for (int y = 0; y < 14; y++) {
          ...
          if ((pixel >>> 24) != 0) ink = Math.max(ink, x + 1);
          if (y >= 2) assertEquals(legacy.getRGB(col * 16 + x, row * 12 + y - 2), pixel);
        }
      }
  ...
  @Test void panelUsesCenteredAlphaCanvasAndMatchingScaledAdvance() throws Exception {
    ...
    for (int y = 0; y < 256; y++) {
      for (int x = 0; x < 256; x++) {
        ...
        if ((pixel >>> 24) != 0) {
          assertTrue(x >= 58 && x < 198 && y >= 104 && y < 152, "Panel must stay centered in its alpha canvas");
          rightmost = Math.max(rightmost, x);
        }
        if (x != 0 || y > 1) assertEquals(pixel, anchored.getRGB(x, y));
      }
    }
    ...
    String json = Files.readString(pack.resolve("assets/vexcore/font/"
        + version.textureFont(UiTexture.PANEL, ScreenAnchor.BOTTOM_CENTER, -143).value() + ".json"));
    assertTrue(json.contains("\"height\":512"));
  }

  @Test void unsupportedAssetsAndDecorationsFailBeforePublishing() {
    ...
    for (var decoration : List.of(TextDecoration.ITALIC, TextDecoration.UNDERLINED, TextDecoration.STRIKETHROUGH)) {
      assertThrows(IllegalArgumentException.class, () -> ScreenUiRenderer.text(
          List.of(Component.text("A").decorate(decoration)), layout, version));
    }
    assertNotNull(ScreenUiRenderer.text(List.of(Component.text("A").decorate(TextDecoration.BOLD)), layout, version));
  }

  @Test void allCardModesHavePackFontsAndNexoSizedTextures() throws Exception {
    for (int height : new int[]{20, 48, 64, 80, 96, 128}) {
      ...
      for (ScreenAnchor anchor : ScreenAnchor.values()) {
        for (boolean compact : new boolean[]{false, true}) {
          ...
          assertTrue(Files.exists(pack.resolve("assets/vexcore/font/"
              + version.textFont(anchor, 0, true, compact).value() + ".json")));
          if (height == 20) assertTrue(Files.exists(pack.resolve("assets/vexcore/font/"
              + version.textureFont(card, anchor, 0, true, compact).value() + ".json")));
        }
      }
    }
  ...
  @Test void scalePresetsResolveForAllGlyphKindsAndSurviveVertexFloatConversion() throws Exception {
