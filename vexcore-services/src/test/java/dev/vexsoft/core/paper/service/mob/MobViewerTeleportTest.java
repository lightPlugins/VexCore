...
public final class MobViewerTeleportTest {
  ...
  public void repeatedDialogueCorrectionsAndCameraRotationDoNotResetNpcs() {
    ...
    for (int i = 0; i < 1000; i++) {
      ...
      corrected.setYaw(i % 360);
      corrected.setPitch(i % 90);
      assertFalse(MobViewerTeleport.changesPosition(new PlayerTeleportEvent(player, from, corrected,
  ...
  public void realRelocationStillResetsEvenForSmallChangesOrTheSameCoordinatesInAnotherWorld() {
    Location from = new Location(null, 20.5, 64, -30.5);
    for (Location to : new Location[]{from.clone().add(0.00001, 0, 0), from.clone().add(0, 1, 0),
        from.clone().add(0, 0, 1000)}) {
      assertTrue(MobViewerTeleport.changesPosition(new PlayerTeleportEvent(player, from, to,
          PlayerTeleportEvent.TeleportCause.PLUGIN)));
    }
