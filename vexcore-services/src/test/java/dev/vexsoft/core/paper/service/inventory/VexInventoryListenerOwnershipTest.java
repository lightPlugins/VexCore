...
public final class VexInventoryListenerOwnershipTest {
    ...
    @Test
    void foreignListenersLeaveLowerInventoryClicksUncancelledInEitherOrder() throws Exception {
        Fixture fixture = new Fixture();

        for (boolean foreignFirst : new boolean[]{true, false}) {
            InventoryClickEvent click = fixture.click(60);

            if (foreignFirst) {
                fixture.foreignListener.onClick(click);
            }

            fixture.ownerListener.onClick(click);
        ...
        assertEquals(2, fixture.menu.lowerClicks);
    }

    @Test
    void foreignListenersCannotSwallowTheNextPageButton() throws Exception {
        Fixture fixture = new Fixture();
        InventoryClickEvent click = fixture.click(44);

        fixture.foreignListener.onClick(click);
        fixture.ownerListener.onClick(click);

        assertTrue(click.isCancelled(), "Menu button must not be taken out");
        assertEquals(1, fixture.menu.page);
    }

    @Test
    void existingProtectionCancellationsRemainEffective() throws Exception {
        Fixture fixture = new Fixture();
        InventoryClickEvent click = fixture.click(44);
        ...
        assertEquals(0, fixture.menu.page);
    }

    @Test
    void foreignCloseEventsCannotRetireAnOwnersCurrentMenu() throws Exception {
        Fixture fixture = new Fixture();
        VexInventoryHolder foreignHolder =
            new VexInventoryHolder(fixture.foreign, fixture.viewerId, InventoryKey.of("test:other"));

        fixture.owner.handleClose(fixture.player, foreignHolder);

        assertSame(fixture.menu, fixture.owner.getSession(fixture.viewerId).getCurrentView());
    }

    @Test
    void foreignDragEventsRemainMovable() throws Exception {
        Fixture fixture = new Fixture();
        InventoryDragEvent drag = new InventoryDragEvent(fixture.view, null, null, false, Map.of());

        fixture.foreignListener.onDrag(drag);
        fixture.ownerListener.onDrag(drag);

        assertFalse(drag.isCancelled());
        assertEquals(1, fixture.menu.drags);
    }
    ...
    private static final class Fixture {
        ...
        Fixture() throws Exception {
            ...
            owner =
                ownerRegistry.require(InventoryService.class) instanceof VexInventoryService service ? service : null;
            ...
            player = proxy(
                Player.class,
                (proxyObject, invokedMethod, arguments) -> invokedMethod.getName().equals("getUniqueId") ? viewerId
                    : null
            );
            ...
            view = proxy(
                ...
                (proxyObject, invokedMethod, arguments) -> switch (invokedMethod.getName()) {
                    ...
                    case "convertSlot" -> (int) arguments[0];
                    default -> null;
                }
            );
    ...
    private static VexServiceRegistry registry() {
        Object plugin = Proxy.newProxyInstance(
            ...
            new Class<?>[]{Plugin.class, ServiceOwner.class},
            (proxyObject, invokedMethod, arguments) -> null
        );
        ScheduleService schedules = proxy(ScheduleService.class, (proxyObject, invokedMethod, arguments) -> null);
        VexInventoryService[] inventory = new VexInventoryService[1];
        VexServiceRegistry registry = proxy(
            VexServiceRegistry.class,
            (proxyObject, invokedMethod, arguments) -> switch (invokedMethod.getName()) {
                case "getOwner" -> plugin;
                case "require" -> arguments[0] == ScheduleService.class ? schedules : inventory[0];
                default -> null;
            }
        );
    ...
    private static final class TestMenu implements MutableInventoryView {
        ...
        public InventoryKey getKey() {
            return InventoryKey.of("test:loadout");
        }
        ...
        public void onInventoryClick(InventoryContext context, InventoryClickEvent event) {
            ...
            if (event.getRawSlot() == 44 && event.getClick() == ClickType.LEFT) {
                page++;
            }
        }
