...
class VerifyShader {
    ...
    static String expand(String source) throws Exception {
        ...
        while (matcher.find()) {
            ...
            String include = Files.exists(pack.resolve(resource)) ? Files.readString(pack.resolve(resource))
                : new String(
                    client.getInputStream(client.getEntry(resource)).readAllBytes(),
                    java.nio.charset.StandardCharsets.UTF_8
                );

            matcher.appendReplacement(
                result,
                Matcher.quoteReplacement(expand(include.replaceAll("(?m)^#version[^\\r\\n]*", "")))
            );
        }
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (String defines : List.of(
            "",
            "IS_GUI",
            "IS_SEE_THROUGH",
            "IS_GRAYSCALE",
            "IS_GUI,IS_GRAYSCALE",
            "IS_SEE_THROUGH,IS_GRAYSCALE"
        )) {
            for (String extension : List.of("vsh", "fsh")) {
                String resource = "assets/minecraft/shaders/core/text." + extension;
                String source = Files.exists(pack.resolve(resource)) ? Files.readString(pack.resolve(resource))
                    : new String(
                        client.getInputStream(client.getEntry(resource)).readAllBytes(),
                        java.nio.charset.StandardCharsets.UTF_8
                    );
                ...
                long result = shaderc_compile_into_spv(
                    compiler,
                    source,
                    extension.equals("vsh") ? shaderc_vertex_shader : shaderc_fragment_shader,
                    resource,
                    "main",
                    options
                );
                ...
                System.out.println("PASS " + extension + " [" + defines + "]");
            }
        }
