...
public record LookAtPlayerGoalDefinition(int priority, double acquireRadius, double releaseRadius,
                                         ...
                                         double switchDistanceAdvantage, boolean requireLineOfSight,
                                         boolean whileMoving, boolean yawOnly) implements MobGoalDefinition {

    /** Creates and validates a look-at-player goal. */
    public LookAtPlayerGoalDefinition {
        if (priority < 0 || !Double.isFinite(acquireRadius) || acquireRadius <= 0.0D || !Double.isFinite(releaseRadius)
            || releaseRadius < acquireRadius || reacquireIntervalTicks < 1 || minimumTargetLockTicks < 0
    ...
    public static final class Builder {
        ...
        public LookAtPlayerGoalDefinition build() {
            return new LookAtPlayerGoalDefinition(
                ...
                acquireRadius,
                releaseRadius,
                reacquireIntervalTicks,
                minimumTargetLockTicks,
                switchDistanceAdvantage,
                requireLineOfSight,
                whileMoving,
                yawOnly
            );
