...
/**
 * Pack contract: fonts {@code <fontPrefix>/y_0} through {@code /y_240} contain the glyph
 * at the declared visible size. glyphAdvance and glyphOffsetX describe transparent canvas padding.
 * The five-argument constructor uses an unpadded glyph with advance width + 1.
 * No runtime file download occurs.
 */
public record UiTexture(Key fontPrefix, int codePoint, int width, int height, int glyphAdvance, int glyphOffsetX) {
  ...
  public UiTexture {
    ...
    if (!Character.isValidCodePoint(codePoint) || codePoint >= 0xD800 && codePoint <= 0xDFFF
        || width < 1 || width > 512 || height < 1 || height > 240
        || glyphAdvance < 1 || glyphAdvance > 1024 || Math.abs((long) glyphOffsetX) > 512) {
