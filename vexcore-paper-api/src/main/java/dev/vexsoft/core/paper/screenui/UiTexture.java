 ...
 * occurs.
 */
public record UiTexture(
    Key fontPrefix, int codePoint, int width, int height, int glyphAdvance, int glyphOffsetX) {
  public static final UiTexture PANEL =
      new UiTexture(Key.key("vexcore:ui/panel"), 0xE100, 280, 96, 397, -116);
  ...
  public static UiTexture card(int height) {
    ...
    return new UiTexture(
        Key.key("vexcore:ui/card_" + height), 0xE100, width, height, width + 9, -8);
  }
