...
public final class PlayerActions {
  ...
  public static void register(ActionRegistry<Player> registry) {
    registry.register("sound", section -> {
      ...
      if (!value.contains(":")) throw new IllegalArgumentException("Sound requires a namespaced key");
      Key key = Key.key(value);
      double volume = section.getDouble("volume", 1);
      double pitch = section.getDouble("pitch", 1);
      if (!Double.isFinite(volume) || volume < 0 || volume > Float.MAX_VALUE
          || !Double.isFinite(pitch) || pitch <= 0 || pitch > Float.MAX_VALUE)
