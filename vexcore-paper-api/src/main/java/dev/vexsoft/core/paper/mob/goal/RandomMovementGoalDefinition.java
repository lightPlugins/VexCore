...
public record RandomMovementGoalDefinition(
    int priority,
    double speed,
    ...
    int retryDelayTicks,
    boolean constrainEntirePath,
    boolean avoidFluids,
    boolean avoidHazards,
    Set<Material> allowedSupportBlocks,
    Set<Material> deniedSupportBlocks
  ...
  public RandomMovementGoalDefinition {
    if (priority < 0 || !Double.isFinite(speed) || speed <= 0.0D
        ...
        || !Double.isFinite(maximumDistance) || maximumDistance <= minimumDistance
    ...
    allowedSupportBlocks = Set.copyOf(Objects.requireNonNull(
        allowedSupportBlocks,
        "allowedSupportBlocks"
    ));
    deniedSupportBlocks = Set.copyOf(Objects.requireNonNull(
        deniedSupportBlocks,
        "deniedSupportBlocks"
    ));
  }
  ...
  public static final class Builder {
    ...
    public RandomMovementGoalDefinition build() {
      return new RandomMovementGoalDefinition(
          priority, speed, minimumDistance, maximumDistance, leashRadius,
          minimumYOffset, maximumYOffset, attempts, minimumIdleTicks, maximumIdleTicks,
          retryDelayTicks, constrainEntirePath, avoidFluids, avoidHazards,
          allowedSupportBlocks, deniedSupportBlocks
      );
    }
  }
}
