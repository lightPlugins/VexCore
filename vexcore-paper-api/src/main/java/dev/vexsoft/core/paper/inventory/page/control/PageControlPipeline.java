...
public class PageControlPipeline {
    ...
    public static <T> List<T> apply(
        ...
        for (PageFilterControl<T> filter : filters) {
            String mode = states.getActiveMode(viewerId, inventoryKey, areaId, filter.getControlId())
                .orElse(filter.getDefaultModeId());

            result = result.stream().filter(filter.getPredicate(mode, inventoryKey, viewerId)).toList();
        }

        Comparator<T> combined = null;

        for (PageSortControl<T> sort : sorts) {
            String mode = states.getActiveMode(viewerId, inventoryKey, areaId, sort.getControlId())
