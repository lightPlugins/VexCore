...
public final class VexCoreLanguageCommand {
    ...
    @Command(value = "language set <language>", permission = "vexcore.command.language", playerOnly = true)
    public int setLanguage(
        ...
        if (selected.isEmpty()) {
            messages.send(player, "commands.vexcore.language.not-found", true, Map.of("language", language));
