...
public final class VexCoreUiCommand {
  ...
  public int dialogue(VexCommandSource source) {
    ...
    var language = players.require(player.getUniqueId()).getContainer(LanguageContainer.class).getLanguage().getKey();
    var speaker = localization.resolve(language, "screen-ui.dialogue-demo.speaker", Map.of()).getComponents().getFirst();
    ...
    try {
      panel.setHint(localization.resolve(language, "screen-ui.dialogue-demo.hint", Map.of()).getComponents().getFirst());
  ...
  public int toggle(VexCommandSource source) {
    ...
    try {
      for (ScreenAnchor anchor : ScreenAnchor.values()) {
        ...
        ui.textBlock(name, localization.resolve(language, "screen-ui.anchors." + name, Map.of()).getComponents(),
            TextBlockLayout.builder().anchor(anchor).x(x).y(y).maxWidth(100)
                .horizontalAlignment(horizontal).verticalAlignment(vertical).layer(10).build());
      }
      messages.send(player, "screen-ui.enabled", true);
      return 1;
    } catch (RuntimeException failure) {
