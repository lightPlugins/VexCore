...
@Dependencies({SendMessageService.class, MessagingService.class, ProxyPingService.class, PerformanceBossBarService.class})
public final class VexCoreDebugCommand {
    ...
    @Command(value = "debug proxy ping", permission = "vexcore.command.debug.proxy.ping", playerOnly = true)
    public int proxyPing(final VexCommandSource source) {
        Player player = (Player) source.getSender();
        UUID requestId = proxyPings.begin(player);
        DeliveryResult result =
            messaging.send(MessageTarget.proxy(), ProxyDebugMessages.PING_REQUEST, new ProxyPingRequest(requestId));
    ...
    @Command(value = "debug performance toggle", permission = "vexcore.command.debug.performance", playerOnly = true)
    public int togglePerformance(final VexCommandSource source) {
        ...
        messages.send(
            player,
            visible ? "commands.vexcore.debug.performance.enabled" : "commands.vexcore.debug.performance.disabled",
            true
        );
