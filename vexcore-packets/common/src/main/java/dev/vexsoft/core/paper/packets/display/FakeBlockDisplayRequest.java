...
public final class FakeBlockDisplayRequest {
     ...
     */
    public static FakeBlockDisplayRequestBuilder builder(final Location location, final BlockData blockData) {
        return internalBuilder().location(location)
            .blockData(blockData)
    ...
    private static FakeBlockDisplayRequest create(
        ...
        return new FakeBlockDisplayRequest(
            ...
            glowing,
            glowColor,
            checkedLifecycle.isEmpty() ? Set.of() : Set.copyOf(EnumSet.copyOf(checkedLifecycle))
        );
    }
}
