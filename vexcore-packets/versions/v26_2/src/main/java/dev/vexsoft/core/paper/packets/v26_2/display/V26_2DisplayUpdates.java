...
public class V26_2DisplayUpdates {
    ...
    public static byte textFlags(final FakeTextDisplayRequest request) {
        ...
        flags = setFlag(flags, Display.TextDisplay.FLAG_USE_DEFAULT_BACKGROUND, request.isDefaultBackground());

        return V26_2DisplayMapper.applyAlignment(flags, request.getAlignment());
    }

    public static void applyText(final Display.TextDisplay display, final FakeTextDisplayUpdate update) {
        ...
        if (update.getDefaultBackground() != null) {
            flags = setFlag(flags, Display.TextDisplay.FLAG_USE_DEFAULT_BACKGROUND, update.getDefaultBackground());
        }
    ...
    public static void applyItem(final Display.ItemDisplay display, final FakeItemDisplayUpdate update) {
    ...
    public static void applyBlock(final Display.BlockDisplay display, final FakeBlockDisplayUpdate update) {
    ...
    private static void applyBase(final Display display, final FakeTextDisplayUpdate update) {
        ...
        applyDimensions(
            display,
            update.getViewRange(),
            update.getShadowRadius(),
            update.getShadowStrength(),
            update.getDisplayWidth(),
            update.getDisplayHeight(),
            update.getInterpolationDelay(),
            update.getInterpolationDuration(),
            update.getTeleportDuration()
        );
    }
