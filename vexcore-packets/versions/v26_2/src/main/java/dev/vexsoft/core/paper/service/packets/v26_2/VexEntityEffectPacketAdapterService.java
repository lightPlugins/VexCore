...
/** Sends viewer-specific hit, glow, lightning, and animation effects using Minecraft 26.2 packets. */
...
public final class VexEntityEffectPacketAdapterService implements EntityEffectPacketAdapterService {
    ...
    public void playHit(final Player viewer, final LivingEntity target) {
        ...
        transport.sendBundle(
            viewer,
            List.of(new ClientboundEntityEventPacket(entity, HURT_EVENT), new ClientboundHurtAnimationPacket(entity))
        );
    }
    ...
    public void setGlow(final Player viewer, final LivingEntity target, final DisplayGlowColor color) {
        ...
        transport.sendBundle(
            ...
            List.of(
                ...
                created ? V26_2GlowPackets.addTeam(entity, color) : V26_2GlowPackets.updateTeam(entity, color)
            )
        );
    }
    ...
    public void strikeLightning(final Player viewer, final LivingEntity target) {
        ...
        LightningBolt lightning =
            new LightningBolt(EntityTypes.LIGHTNING_BOLT, ((CraftWorld) target.getWorld()).getHandle());
        ...
        transport.send(
            ...
            new ClientboundAddEntityPacket(
                ...
                lightning.getType(),
                0,
                lightning.getDeltaMovement(),
                lightning.getYHeadRot()
            )
        );
    }
