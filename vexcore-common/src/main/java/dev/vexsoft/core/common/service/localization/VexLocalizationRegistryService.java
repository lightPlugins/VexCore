...
/** Loads owner-specific language resources and resolves localized messages with placeholders and theme colors. */
...
public final class VexLocalizationRegistryService implements LocalizationRegistryService {
    ...
    public VexLocalizationRegistryService(final VexServiceRegistry services) {
        ...
        staticMessages = checkedServices.require(CacheService.class).create(
            "localization-static-messages",
            VexCacheOptions.builder().maximumSize(20_000L).expireAfterAccess(Duration.ofMinutes(30)).build()
        );
    }
    ...
    public LocalizedMessage resolve(
        ...
        if (registration.owner != owner) {
            throw new IllegalStateException(
                "Localization owner instance is no longer registered: " + owner.getServiceOwnerName());
        }
    ...
    public Collection<LocalizationOwner> getOwners() {
        return registrations.values()
            ...
            .sorted((left, right) -> left.getServiceOwnerName().compareToIgnoreCase(right.getServiceOwnerName()))
            .toList();
    }
    ...
    private LocalizedMessage render(
        ...
        if (checkedReplacements.isEmpty()) {
            StaticMessageKey cacheKey =
                new StaticMessageKey(normalizeOwner(registration.owner.getServiceOwnerName()), language, checkedKey);
    ...
    private LocalizedMessage renderTemplate(
        ...
        List<Component> components =
            template.getLines().stream().map(line -> themeColors.deserialize(replace(line, replacements))).toList();

        return template.isList() ? LocalizedMessage.list(components) : LocalizedMessage.single(components.getFirst());
    }
    ...
    private String replace(final String template, final Map<String, String> replacements) {
        ...
        while (matcher.find()) {
            ...
            String replacement = value == null ? matcher.group() : miniMessage.escapeTags(value);
