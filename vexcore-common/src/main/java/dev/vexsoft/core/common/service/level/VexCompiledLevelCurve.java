...
final class VexCompiledLevelCurve implements CompiledLevelCurve {
    ...
    VexCompiledLevelCurve(final int minimumLevel, final int maximumLevel, final double[] thresholds) {
    ...
    public LevelSnapshot calculate(final double experience) {
        ...
        if (maximum) {
            return new LevelSnapshot(level, experience, start, start, experience - start, 0.0D, 1.0D, true);
        }
        ...
        return new LevelSnapshot(level, experience, start, next, inLevel, required, inLevel / required, false);
    }
