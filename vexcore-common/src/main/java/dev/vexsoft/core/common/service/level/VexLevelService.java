...
public final class VexLevelService implements LevelService {
    ...
    public CompiledLevelDefinition compile(final LevelDefinition definition) {
        ...
        for (LevelRuleDefinition rule : definition.rules()) {
            if (rule.minimumLevel() <= definition.minimumLevel() || rule.minimumLevel() > definition.maximumLevel()) {
        ...
        return new CompiledLevelDefinition(
            new VexCompiledLevelCurve(definition.minimumLevel(), definition.maximumLevel(), thresholds),
    ...
    public CompiledLevelDefinition compile(final ConfigurationSection configuration) {
        ...
        String expression =
            Objects.requireNonNull(leveling.getString("experience.required"), "Missing leveling.experience.required");
        LevelClaimMode mode = parseEnum(LevelClaimMode.class, leveling.getString("claim-mode", "MANUAL"));
        ClaimedLevelOverflowPolicy overflow =
            parseEnum(ClaimedLevelOverflowPolicy.class, leveling.getString("claimed-level-overflow", "KEEP"));
    ...
    private static ConfigurationSection requireSection(final ConfigurationSection configuration, final String path) {
