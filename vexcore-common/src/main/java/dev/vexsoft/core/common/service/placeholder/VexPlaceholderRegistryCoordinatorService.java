...
@Dependencies
public final class VexPlaceholderRegistryCoordinatorService implements PlaceholderRegistryCoordinatorService {

    private static final Pattern TOKEN = Pattern.compile("%([A-Za-z0-9_]+)%");
    private final Map<String, RegisteredPlaceholder> registrations = new LinkedHashMap<>();
    private final Cache<String, CompiledTemplate> templates = Caffeine.newBuilder().maximumSize(4096).build();
    ...
    public synchronized <T extends VexPlaceholder> T register(
        ...
        if (annotation == null) {
            throw new IllegalArgumentException("Placeholder is missing @PlaceholderId: " + checkedType.getName());
        }
    ...
    private synchronized void rebuildSnapshot() {
        snapshot = registrations.values()
            .stream()
            .sorted(Comparator.comparingInt((RegisteredPlaceholder value) -> value.key().length()).reversed())
            .toList();
    ...
    private interface TemplatePart {
        ...
        /** Appends this template segment after resolving its placeholders against the supplied context. */
    ...
    private record PlaceholderPart(String source, String name, ResolvedRegistration registration) implements
        TemplatePart {
        ...
        public void append(final StringBuilder output, final PlaceholderContext context) {
            ...
            String resolved = registration.placeholder().resolve(context.getPlayer(), registration.arguments());

            output.append(resolved == null ? source : resolved);
        }
    }

    private record RegisteredPlaceholder(ServiceOwner owner, String key, VexPlaceholder placeholder) {

    }

    private record ResolvedRegistration(VexPlaceholder placeholder, PlaceholderArguments arguments) {
