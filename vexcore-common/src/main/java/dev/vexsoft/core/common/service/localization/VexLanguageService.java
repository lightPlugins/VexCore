...
/** Resolves available languages, installs player language containers, and reloads localization resources. */
@Dependencies({LocalizationRegistryService.class, LanguageChangeDispatcherService.class, PlayerContainerService.class})
public final class VexLanguageService implements LanguageService {
    ...
    public Optional<Language> findLanguage(final String language) {
        ...
        return localizations.getLanguages(CORE_OWNER).contains(key) ? Optional.of(language(key)) : Optional.empty();
    }

    @Override
    public Collection<Language> getLanguages() {
        return localizations.getLanguages(CORE_OWNER).stream().sorted().map(this::language).toList();
    }
    ...
    private Language requireLanguage(final LanguageKey key) {
        return findLanguage(key.getValue()).orElseThrow(() -> new IllegalArgumentException(
            "Language is not available: " + key));
    }
