...
@Dependencies
public final class VexExecutionComponentCoordinatorService implements ExecutionComponentCoordinatorService,
    AutoCloseable {
    ...
    public synchronized void register(
        ...
        if (existing != null) {
            throw new IllegalStateException(
                "Duplicate " + kind.name().toLowerCase() + " key '" + checkedKey + "' owned by " + existing.owner()
                    .getServiceOwnerName());
        }
    ...
    @Override
    public synchronized Optional<Object> find(final ExecutionComponentKind kind, final String key) {
    ...
    @Override
    public synchronized void unregisterOwner(final ServiceOwner owner, final ExecutionComponentKind kind) {
