...
public interface PlayerDataStore extends AutoCloseable {
    ...
    CompletableFuture<Map<String, String>> load(String owner, UUID uniqueId, Collection<DataContainerKey<?>> keys);

    /** Saves JSON values for one plugin and player */
    CompletableFuture<Void> save(String owner, UUID uniqueId, String playerName, Map<String, String> values);
    ...
    default CompletableFuture<Void> saveAllOwners(
        ...
        if (owners.size() != 1) {
            return CompletableFuture.failedFuture(new UnsupportedOperationException(
                "Atomic multi-owner saves are not supported"));
        }
    ...
    default CompletableFuture<Integer> reset(String owner, UUID uniqueId, Collection<String> containers) {
        return CompletableFuture.failedFuture(new UnsupportedOperationException("Player data resets are not supported"));
    }

    /** Resets selected containers for every stored player of one owner. */
    default CompletableFuture<Integer> resetAll(String owner, Collection<String> containers) {
        return CompletableFuture.failedFuture(new UnsupportedOperationException(
            "Global player data resets are not supported"));
    }

    /** Finds the most recently stored identity matching a case-insensitive player name. */
    default CompletableFuture<Optional<UUID>> findUniqueId(String owner, String playerName) {
        return CompletableFuture.completedFuture(Optional.empty());
    }

    /** Closes the storage backend and releases its resources. */
    @Override
    void close();
}
