...
public interface PlayerDataStore extends AutoCloseable {
  ...
  CompletableFuture<Map<String, String>> load(
      String owner, UUID uniqueId, Collection<DataContainerKey<?>> keys);

  /** Saves JSON values for one plugin and player */
  CompletableFuture<Void> save(
      String owner, UUID uniqueId, String playerName, Map<String, String> values);
  ...
  default CompletableFuture<Integer> reset(
      String owner, UUID uniqueId, Collection<String> containers) {
    return CompletableFuture.failedFuture(
