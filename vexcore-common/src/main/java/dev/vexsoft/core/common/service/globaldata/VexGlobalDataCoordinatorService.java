...
public final class VexGlobalDataCoordinatorService implements GlobalDataCoordinatorService, AutoCloseable {
    ...
    private final Map<GlobalDataReference, RegisteredGlobalData> registrations = new LinkedHashMap<>();
    ...
    public VexGlobalDataCoordinatorService(final VexServiceRegistry services) {
        ...
        cache = checkedServices.require(CacheService.class).createAsync(
            "global-data",
            VexCacheOptions.builder().maximumSize(10_000L).expireAfterAccess(Duration.ofMinutes(10)).build(),
            reference -> store.loadGlobalData(reference.owner(), reference.key()).thenApply(GlobalCacheEntry::new)
        );
        invalidationSubscription = store.subscribeGlobalDataChanges(cache::invalidate);
    }

    @Override
    public synchronized void register(final ServiceOwner owner, final GlobalDataDefinition definition) {
    ...
    @Override
    public <T> CompletableFuture<T> get(final ServiceOwner owner, final GlobalDataKey<T> key) {
    ...
    @Override
    public <T> CompletableFuture<T> refresh(final ServiceOwner owner, final GlobalDataKey<T> key) {
        RegisteredGlobalData registration = requireRegistration(owner, key);
    ...
    @Override
    public <T> CompletableFuture<Void> set(final ServiceOwner owner, final GlobalDataKey<T> key, final T value) {
        RegisteredGlobalData registration = requireRegistration(owner, key);
        String json = serialize(key, value);

        return store.setGlobalData(registration.reference().owner(), registration.reference().key(), json)
            .thenAccept(stored -> cache.put(registration.reference(), new GlobalCacheEntry(Optional.of(stored))));
    }
    ...
    @Override
    public CompletableFuture<Boolean> reset(final ServiceOwner owner, final GlobalDataKey<?> key) {
        RegisteredGlobalData registration = requireRegistration(owner, key);

        return store.resetGlobalData(registration.reference().owner(), registration.reference().key())
            .thenApply(removed -> {
                cache.invalidate(registration.reference());
    ...
    private synchronized void registerKey(final String owner, final GlobalDataKey<?> key) {
        ...
        RegisteredGlobalData existing =
            registrations.putIfAbsent(reference, new RegisteredGlobalData(reference, checkedKey));
    ...
    private <T> CompletableFuture<T> update(
        ...
        return store.loadGlobalData(reference.owner(), reference.key()).thenCompose(current -> {
            ...
            return store.compareAndSetGlobalData(reference.owner(), reference.key(), revision, json)
                .thenCompose(stored -> {
                    if (stored.isPresent()) {
                    ...
                    if (attempt >= MAX_UPDATE_ATTEMPTS) {
                        return CompletableFuture.failedFuture(new IllegalStateException(
                            "Global data changed too frequently to update: " + reference.owner() + ':'
                                + reference.key()));
                    }
    ...
    private <T> T deserialize(final GlobalDataKey<T> key, final GlobalCacheEntry entry) {
        ...
        try {
            ...
            throw new IllegalStateException("Stored global data has an invalid value for " + key.getName(), exception);
        }
    }
    ...
    private <T> String serialize(final GlobalDataKey<T> key, final T value) {
        ...
        try {
            ...
            throw new IllegalArgumentException("Unable to serialize global data " + key.getName(), exception);
        }
    }

    private String ownerName(final ServiceOwner owner) {
        String name =
            Objects.requireNonNull(owner, "owner").getServiceOwnerName().toLowerCase(Locale.ROOT).replace('-', '_');
