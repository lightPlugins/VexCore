...
/** Provides access to the fully loaded players managed by the shared data coordinator. */
...
public final class VexPlayerService implements PlayerService {
    ...
    public VexPlayerService(final VexServiceRegistry services) {
        this.coordinator = Objects.requireNonNull(services, "services").require(PlayerDataCoordinatorService.class);
    }
    ...
    public VexPlayer require(final UUID uniqueId) {
        return find(uniqueId).orElseThrow(() -> new IllegalStateException("VexPlayer is not loaded: " + uniqueId));
    }
}
