...
/** Serializes typed messages and manages this owner's handlers on the shared transport. */
...
public final class VexMessagingService implements MessagingService, AutoCloseable {
    ...
    @Override
    public <T> DeliveryResult send(final MessageTarget target, final MessageType<T> type, final T message) {
    ...
    public void register(final Class<? extends MessageHandler<?>> handlerType) {
        MessageHandler<?> handler =
            VexClassFactory.create(Objects.requireNonNull(handlerType, "handlerType"), services, "Message handler");

        handlers.computeIfAbsent(handler.getMessageType().getKey(), ignored -> new CopyOnWriteArrayList<>())
            .add(handler);
    }
