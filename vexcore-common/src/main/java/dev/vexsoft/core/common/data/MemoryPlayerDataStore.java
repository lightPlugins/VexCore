...
public final class MemoryPlayerDataStore implements PlayerDataStore, GlobalDataStore, PlayerIdentityStore {
    ...
    public synchronized CompletableFuture<Map<String, String>> load(
        ...
        Map<String, String> stored = values.getOrDefault(owner, Map.of()).getOrDefault(uniqueId, Map.of());
    ...
    public CompletableFuture<Optional<UUID>> findUniqueId(final String owner, final String playerName) {
        return CompletableFuture.completedFuture(names.getOrDefault(owner, Map.of())
            .entrySet()
            .stream()
    ...
    public CompletableFuture<Optional<StoredGlobalData>> loadGlobalData(final String owner, final String key) {
        return CompletableFuture.completedFuture(Optional.ofNullable(globalValues.get(new GlobalDataReference(
            owner,
            key
        ))));
    }
