...
/** Initializes the configured storage backend shared by player data, global data, and identities. */
...
public final class VexPlayerDataStoreService implements PlayerDataStoreService, AutoCloseable {
    ...
    public VexPlayerDataStoreService(final VexServiceRegistry services) {
        ...
        if (loginTimeoutSeconds < 1L || loginTimeoutSeconds > 120L) {
            throw new IllegalArgumentException("player-data.login-timeout-seconds must be between 1 and 120");
        }
        ...
        store = switch (storageType) {
            ...
            default -> throw new IllegalArgumentException("Unsupported player data storage: " + storageType);
        };
    ...
    private Path sqliteFile(final Path configurationDirectory, final String configuredFile) {
        Path root =
            Objects.requireNonNull(configurationDirectory, "configurationDirectory").toAbsolutePath().normalize();
