...
public final class VexCacheOptionsTest {
    ...
    public void rejectsUnboundedOrImmediateCaches() {
        assertThrows(IllegalArgumentException.class, () -> VexCacheOptions.builder().maximumSize(0).build().validate());
        assertThrows(
            IllegalArgumentException.class,
            () -> VexCacheOptions.builder().expireAfterWrite(Duration.ZERO).build().validate()
        );
    }
}
