...
class VexLocalizedMessageServiceTest {
    ...
    void sendsPlayerLanguageAndExplicitAudienceMessagesThroughOneImplementation() {
        Language german = new Language(LanguageKey.of("de_DE"), Component.text("Deutsch"), false);
        ...
        VexPlayer player = new VexPlayer(UUID.randomUUID(), "Alex", type -> type == LanguageContainer.class ? 0 : -1);

        player.installContainer(
            0,
        ...
        List<String> rendered = received.stream().map(PlainTextComponentSerializer.plainText()::serialize).toList();
