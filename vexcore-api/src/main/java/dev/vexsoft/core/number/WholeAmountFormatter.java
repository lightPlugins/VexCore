...
public final class WholeAmountFormatter {
  ...
  public static WholeAmount parse(final String input) {
    try {
      ...
      throw new IllegalArgumentException("Compact whole amount has a fractional result: " + input,
          exception);
    }
  }
  ...
  public static BigDecimal parseDecimal(final String input) {
    ...
    BigDecimal expanded = new BigDecimal(matcher.group(1))
        .multiply(new BigDecimal(BigInteger.TEN.pow(Math.multiplyExact(group, 3))));
    return expanded;
  }
  ...
  private static int alphabeticIndex(final String suffix) {
    if (suffix.length() < 2 || !suffix.chars().allMatch(character ->
