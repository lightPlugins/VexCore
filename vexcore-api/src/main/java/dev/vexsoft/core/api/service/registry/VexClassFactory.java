...
public class VexClassFactory {
    ...
    public static <T> T create(final Class<T> type, final VexServiceRegistry services, final String role) {
        ...
        if (!Modifier.isPublic(type.getModifiers()) || Modifier.isAbstract(type.getModifiers())) {
            throw new IllegalArgumentException(checkedRole + " class must be public and concrete: " + type.getName());
        }

        Dependencies dependencies = type.getAnnotation(Dependencies.class);

        if (dependencies == null) {
            throw new IllegalArgumentException(checkedRole + " is missing @Dependencies: " + type.getName());
        }
