...
public interface LocalizedMessageService extends VexService {
    ...
    default void send(final VexPlayer player, final String key, final Map<String, String> replacements) {
        send(player, key, false, replacements);
    }

    /** Sends a localized message using the language selected by a loaded Vex player. */
    void send(VexPlayer player, String key, boolean withPrefix, Map<String, String> replacements);
