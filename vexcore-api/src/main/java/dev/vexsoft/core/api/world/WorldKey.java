...
public record WorldKey(String namespace, String value) {
    ...
    public static WorldKey parse(final String value) {
        ...
        if (separator <= 0 || separator == checkedValue.length() - 1 || checkedValue.indexOf(':', separator + 1) >= 0) {
            throw new IllegalArgumentException("World ID must use the format namespace:value: " + checkedValue);
        }

        return new WorldKey(checkedValue.substring(0, separator), checkedValue.substring(separator + 1));
    }
    ...
    private static String normalizeNamespace(final String namespace) {
        String checkedNamespace = Objects.requireNonNull(namespace, "namespace").trim().toLowerCase(Locale.ROOT);
    ...
    private static String normalizeValue(final String value) {
        String checkedValue = Objects.requireNonNull(value, "value").trim().toLowerCase(Locale.ROOT);
