...
public record StatRewardContribution(Map<StatKey, StatModifier> modifiers) implements RewardContribution {
    ...
    public RewardContribution merge(final RewardContribution other) {
        ...
        stats.modifiers.forEach((key, modifier) -> merged.merge(
            key,
            modifier,
            (left, right) -> {
                ...
                return switch (left.operation()) {
                    case FLAT, ADDITIVE_MULTIPLIER ->
                        new StatModifier(left.operation(), left.amount() + right.amount());
