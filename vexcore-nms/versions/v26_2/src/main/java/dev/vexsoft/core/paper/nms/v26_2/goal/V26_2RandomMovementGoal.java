...
public final class V26_2RandomMovementGoal extends Goal {
    ...
    private Pathfinder.PathResult findTargetPath() {
        ...
        for (int attempt = 0; attempt < spec.attempts(); attempt++) {
            ...
            double distance = ThreadLocalRandom.current().nextDouble(spec.minimumDistance(), spec.maximumDistance());
    ...
    private Location findGround(final World world, final double x, final double z, final int preferredY) {
    ...
    private boolean validStandLocation(final World world, final double x, final int y, final double z) {
        int blockX = (int) Math.floor(x);
        int blockZ = (int) Math.floor(z);
        ...
        if (!GroundPositionSafety.isSafe(
            ...
            spec.avoidFluids(),
            spec.avoidHazards(),
            spec.allowedSupportBlocks(),
            spec.deniedSupportBlocks()
        )) {
            return false;
        }
        ...
        return handle.level().noCollision(
            handle,
    ...
    private int randomIdleTicks() {
        return ThreadLocalRandom.current().nextInt(spec.minimumIdleTicks(), spec.maximumIdleTicks() + 1);
    }

    private static Location centered(final World world, final double x, final int y, final double z) {
    ...
    private static double horizontalDistanceSquared(final Location first, final double x, final double z) {
