...
public class VexItemComponentAdapterService implements ItemComponentAdapterService {
  ...
  protected void apply(
      final ItemStack itemStack, final VexComponentKey key, final VexComponentOperation operation) {
    switch (key) {
      case MAX_STACK_SIZE ->
          applyValue(itemStack, DataComponentTypes.MAX_STACK_SIZE, operation, Integer.class);
      case DAMAGE -> applyValue(itemStack, DataComponentTypes.DAMAGE, operation, Integer.class);
      case MAX_DAMAGE ->
          applyValue(itemStack, DataComponentTypes.MAX_DAMAGE, operation, Integer.class);
      case ENCHANTMENT_GLINT ->
          applyValue(
              itemStack, DataComponentTypes.ENCHANTMENT_GLINT_OVERRIDE, operation, Boolean.class);
      case ITEM_MODEL -> applyValue(itemStack, DataComponentTypes.ITEM_MODEL, operation, Key.class);
      case TOOLTIP_STYLE ->
          applyValue(itemStack, DataComponentTypes.TOOLTIP_STYLE, operation, Key.class);
      case RARITY -> applyValue(itemStack, DataComponentTypes.RARITY, operation, ItemRarity.class);
      case DYED_COLOR, TRIM, ATTRIBUTE_MODIFIERS, TOOLTIP_DISPLAY ->
          applyAppearance(itemStack, key, operation);
  ...
  private void applyCustomModelData(
      final ItemStack itemStack, final VexComponentOperation operation) {
    if (operation.getType() != VexComponentOperationType.SET) {
    ...
    CustomModelData.Builder builder =
  ...
  private void applyAppearance(
      ItemStack stack, VexComponentKey key, VexComponentOperation operation) {
    if (operation.getType() != VexComponentOperationType.SET) {
    ...
    switch (key) {
      ...
      case ATTRIBUTE_MODIFIERS -> {
        ...
        ((VexItemAttributes) operation.getValue())
            .entries()
      ...
      case TOOLTIP_DISPLAY -> {
        ...
        display
            .hiddenComponents()
            .forEach(hidden -> builder.addHiddenComponents(appearanceComponent(hidden)));
